...
dataset_names = [
    ...
    "math_qa",
    "lucasmccabe/logiqa",
    "hagara/labelled-PubMedQA",
    "squad",
    "google/boolq",
    "openai_humaneval",
]
dataset_names = ["lambada", "codeparrot/apps", "mbpp"]
datasets_dir = {
    "lukaemon/mmlu": [
        "astronomy",
        "business_ethics",
...
# tokenizers = ["openai", "mistralai/Mistral-7B-v0.1", "codellama/CodeLlama-7b-hf", "mistralai/Mixtral-8x7B-v0.1"]
...
def get_tokens(dataset, tokenizer):
    if tokenizer == "openai":
        ...
        if "meta-llama" in tokenizer:
            tokenizer = AutoTokenizer.from_pretrained(
                "meta-llama/Llama-2-13b-chat-hf",
                token="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",
            )
        else:
            tokenizer = AutoTokenizer.from_pretrained(tokenizer)
        dataset = dataset.map(
            lambda batch: encode(batch, tokenizer), batched=True, num_proc=num_cores
        )
...
tokenizer_tokens = {
    "openai": 10430743,
    "mistralai/Mistral-7B-v0.1": 13933333,
    "codellama/CodeLlama-7b-hf": 14183549,
    "mistralai/Mixtral-8x7B-v0.1": 13924809,
}
