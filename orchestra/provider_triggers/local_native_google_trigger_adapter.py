"""Local native Google trigger adapter for credential-free stacks and tests."""
...
NATIVE_GOOGLE_WEBHOOK_SECRET_REF = "env:NATIVE_GOOGLE_WEBHOOK_SECRET"
...
@dataclass
class LocalNativeGoogleTriggerScenario:
    """Process-local knobs for native Google trigger tests."""

    account_email: str = "meet.user@example.com"
    display_label: str = "google_meet:meet.user@example.com"
...
_SCENARIO = LocalNativeGoogleTriggerScenario()


def get_local_native_google_trigger_scenario() -> LocalNativeGoogleTriggerScenario:
    return _SCENARIO


def reset_local_native_google_trigger_state() -> None:
    _SCENARIO.reset()
...
def _stable_external_trigger_id(request: TriggerProvisionRequest) -> str:
    ...
    return f"ng_{uuid.uuid5(uuid.NAMESPACE_OID, stable_payload).hex[:12]}"
...
class LocalNativeGoogleTriggerAdapter(TriggerProviderAdapter):
    """Deterministic native Google trigger adapter for local stacks."""
    ...
    def normalize_delivery(
        ...
        *,
        ...
        if not isinstance(payload, dict):
            raise ValueError("Native Google delivery payload must be a JSON object")

        event_id = native_event_identity(payload)
        if not event_id:
            raise ValueError(
                "Native Google delivery is missing retry-stable event identity",
            )
        provider_trigger_slug = str(
            payload.get("provider_trigger_slug") or NATIVE_GOOGLE_MEET_TRANSCRIPT_SLUG,
        )
