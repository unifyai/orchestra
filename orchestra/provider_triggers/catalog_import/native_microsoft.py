"""Microsoft Graph change-notification catalog importer."""
...
class NativeMicrosoftTriggerCatalogImporter:
    """Import Microsoft Graph subscription resources for staging."""
        ...
        *,
        ...
        if force_fixture is None:
            force_fixture = os.getenv(
                "NATIVE_MICROSOFT_CATALOG_FORCE_FIXTURE",
                "",
    ...
    def list_trigger_catalog_entries(self) -> list[ProviderTriggerCatalogEntry]:
        if self._force_fixture:
            if not _fixture_import_allowed(self.environment):
                raise RuntimeError(
                    "NATIVE_MICROSOFT_CATALOG_FORCE_FIXTURE is only allowed in selfhost",
                )
        ...
        try:
            ...
            if not _fixture_import_allowed(self.environment):
                raise RuntimeError(
                    "Native Microsoft catalog manifest missing for non-selfhost import",
                ) from None
            _, entries = load_fixture_catalog(self.backend_id)
            return entries
