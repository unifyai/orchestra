...
def _xdist_database_suffix(worker_id) -> str:
...
def create_database(worker_id=None) -> None:
    ...
    suffix = _xdist_database_suffix(worker_id)
    datname = f"{settings.db_base}{suffix}"
    if suffix:
        url = url.replace("orchestra_test", f"orchestra_test{suffix}")
    db_url = make_url(url)
