...
class AsyncProjectDAO:
    ...
    async def delete(self, id: int):
        try:
            project = (
                (await self.session.execute(select(Project).filter_by(id=id)))
                .scalars()
                .one()
            )
            ...
            soft_delete_result = await self.session.execute(
                update(Embedding)
    ...
    async def filter_by_user_access(
        self,
        user_id: str,
        organization_id: Optional[int] = None,
        id: Optional[int] = None,
    ...
    async def get_by_user_and_name_any_context(
        self,
        user_id: str,
        name: str,
    ) -> Optional[Project]:
    ...
    async def commit(
        ...
        await self.session.flush()  # Flush to get the project_version.id
    ...
    async def rollback(self, project_id: int, commit_hash: str) -> None:
        """
    ...
    async def get_commit_history(self, project_id: int) -> List[dict]:
        """
