"""Async version of resource_access_dao for use with AsyncSession."""

"""Async Data Access Object for ResourceAccess model."""
...
class AsyncResourceAccessDAO:
    ...
    @classmethod
    async def clear_permission_cache(cls):
    ...
    async def revoke_access(
        ...
        """
        query = select(ResourceAccess).filter(
            ResourceAccess.resource_type == resource_type,
            ResourceAccess.resource_id == resource_id,
            ResourceAccess.grantee_type == grantee_type,
            ResourceAccess.grantee_id == grantee_id,
    ...
    async def get(self, access_id: int) -> Optional[ResourceAccess]:
        """
        ...
        """
        return (
            (await self.session.execute(select(ResourceAccess).filter_by(id=access_id)))
            .scalars()
            .first()
        )
    ...
    async def check_user_has_permission_in_org(
        self,
        user_id: str,
        organization_id: int,
        permission_name: str,
    ...
    async def check_org_member_permission(
        self,
        user_id: str,
        organization_id: int,
        permission_name: str,
    ...
    async def _is_personal_resource(self, resource_type: str, resource_id: int) -> bool:
        """
        ...
        if resource_type == "project":
            project = (
                (await self.session.execute(select(Project).filter_by(id=resource_id)))
                .scalars()
                .first()
            )
            ...
            assistant = (
                (
                    await self.session.execute(
                        select(Assistant).filter_by(agent_id=resource_id),
                    )
                )
                .scalars()
                .first()
    ...
    async def _check_personal_ownership(
        self,
        resource_type: str,
        resource_id: int,
        user_id: str,
    ...
    async def _get_resource_organization_id(
        self,
        resource_type: str,
        resource_id: int,
    ) -> Optional[int]:
    ...
    async def revoke_user_access_for_organization(
        self,
        user_id: str,
        organization_id: int,
    ) -> int:
    ...
    async def delete_unshared_resources_by_creator(
        self,
        user_id: str,
        organization_id: int,
    ) -> dict:
    ...
    async def _delete_assistant_logs(
        self,
        assistant: Assistant,
        organization_id: int,
    ) -> None:
    ...
    async def _is_resource_unshared(
        self,
        resource_type: str,
        resource_id: int,
        creator_id: str,
