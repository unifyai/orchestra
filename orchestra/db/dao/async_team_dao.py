"""Async version of team_dao for use with AsyncSession."""

"""Async Data Access Object for Team model."""
...
class AsyncTeamDAO:
    ...
    async def get(self, id: int) -> Optional[Team]:
        ...
        """
        return (
            (await self.session.execute(select(Team).filter_by(id=id)))
            .scalars()
            .first()
        )
    ...
    async def list_organization_teams(self, organization_id: int) -> List[Team]:
        """
    ...
    async def add_member(self, team_id: int, user_id: str) -> TeamMember:
        """
    ...
    async def remove_member(self, team_id: int, user_id: str) -> None:
        """
    ...
    async def get_team_members(self, team_id: int) -> List[str]:
        ...
        """
        members = select(TeamMember).filter_by(team_id=team_id).all()
        return [member.user_id for member in members]

    async def get_user_teams(self, user_id: str, organization_id: int) -> List[Team]:
        """
    ...
    async def remove_user_from_all_org_teams(
        self,
        user_id: str,
        organization_id: int,
    ) -> int:
