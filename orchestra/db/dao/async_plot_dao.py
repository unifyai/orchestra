"""Async version of plot_dao for use with AsyncSession."""
...
class AsyncPlotDAO:
    ...
    async def get_by_token(self, token: str) -> Optional[Plot]:
        """
        ...
        """
        return (
            (await self.session.execute(select(Plot).where(Plot.token == token)))
            .scalars()
            .first()
        )

    async def get_by_id(self, plot_id: int) -> Optional[Plot]:
        """
        ...
        """
        return (
            (await self.session.execute(select(Plot).where(Plot.id == plot_id)))
            .scalars()
            .first()
        )

    async def list_by_project(self, project_id: int) -> List[Plot]:
        """
    ...
    async def list_by_user_context(
        self,
        user_id: str,
        organization_id: Optional[int],
    ...
    async def delete_by_project(
        ...
        """
        query = select(Plot).filter(Plot.project_id == project_id)
    ...
    async def update_organization_id(
        self,
        project_id: int,
        organization_id: Optional[int],
    ) -> int:
