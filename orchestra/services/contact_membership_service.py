...
def ensure_team_contact_memberships(
    session: Session,
    assistant_team_pairs: Sequence[tuple[int, int]],
    *,
    repair_existing: bool = True,
) -> None:
    """Ensure assistants have team-scoped self and boss contact overlays."""

    if not assistant_team_pairs:
        return

    normalized_pairs = {
        (int(assistant_id), int(team_id))
        for assistant_id, team_id in assistant_team_pairs
    }
    if not normalized_pairs:
        return
    ...
    live_pairs = {
        (int(assistant_id), int(team_id))
        for assistant_id, team_id in session.query(
            TeamAssistantMembership.assistant_id,
            TeamAssistantMembership.team_id,
        )
        .filter(
            tuple_(
                TeamAssistantMembership.assistant_id,
    ...
    if repair_existing:
        ...
        for row in existing_rows:
            if row.target_team_id is None:
                continue
            rows_by_pair.setdefault(
                (int(row.assistant_id), int(row.target_team_id)),
                [],
            ).append(row)
    ...
    membership_values = []
    for assistant_id, team_id in sorted(live_pairs):
        rows = rows_by_pair.get((assistant_id, team_id), [])
