...
MY_COMPUTER_FRAMING = (
    ...
    "workspace with a dated filename; (4) open the GUI file manager at that folder so "
    "the file is visibly there — this is the filesystem reveal, a core part of the "
    "step; (5) open the image full-screen in the image viewer; (6) send it to the "
    "user as a send_unify_message attachment with one short caption line. File "
    "handling goes through the GUI file manager only — never open a terminal, never "
    "shell commands like xdg-open. If the user asks for something else mid-call, "
    "honor it as long as it keeps the same shape (real site → download → filesystem "
    "reveal → deliver). "
    ...
    "Rule 5 — Explicit completion: after the attachment is delivered, call "
    "set_onboarding_task_state('my-computer-demo', True), then stop the persist act. "
    "The checklist does not auto-detect anything. "
...
def _my_computer_beat_event(step_id: str, title: str) -> OnboardingEventSpec:
    ...
    return OnboardingEventSpec(
        ...
        message=(
            ...
            "click Show assistant screen. Drive six substeps one at a time — act "
            ...
            "GUI file-manager reveal; open image full-screen; send_unify_message "
            "attachment. Never a terminal or shell xdg-open. After attachment: "
            "set_onboarding_task_state('my-computer-demo', True), stop act, wrap up "
            "from the live progress block (recap, name next step, continue on call or "
            "message). Unanswered ring: one ring-back invite; do not re-ring. On "
            "failure: say so, do not mark done. This is a poll, not a request to "
            "repeat work already done: if the demo is already finished, treat this as "
            "confirmation and do NOT redo it. "
