...
MY_COMPUTER_FRAMING = (
    ...
    "the ack, then branch on its status in that same turn. "
    "If status is 'ready': do NOT say the computer is still booting and do NOT wait "
    "for a desktop-ready notification (none will arrive). Send ONE short chat line "
    "that the computer is ready and you are ringing now, then call start_unify_meet "
    "in that same turn. "
    "If status is 'warming': send ONE chat message — a 2-sentence tutorial intro "
    "(this step shows I have a real computer of my own; you'll watch me use it live) "
    "plus 'booting my computer now — I'll ring you the moment it's up' — then wait; "
    "ring via start_unify_meet only when the desktop-ready notification lands. "
    ...
    "Rule 2 — On-call click: call prepare_desktop if needed. If it returns 'warming', "
    "narrate the boot honestly (~30–60s; the boot is part of the demo — it proves a "
    "real machine) and start the demo when the desktop-ready notification lands. If "
    "it returns 'ready' (or the desktop is already usable), start the demo directly "
    "and tell them to click Show assistant screen so they watch live. "
...
def _my_computer_beat_event(step_id: str, title: str) -> OnboardingEventSpec:
    ...
    return OnboardingEventSpec(
        ...
        message=(
            ...
            "Off-call: call prepare_desktop immediately in the same turn as the ack. "
            "If it returns status 'ready', do not narrate booting and do not wait for "
            "a desktop-ready notification — send one short 'ready, ringing now' chat "
            "line and call start_unify_meet in that same turn. If it returns 'warming', "
            "send the tutorial intro plus 'booting… I'll ring you when it's up', then "
