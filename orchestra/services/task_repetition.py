"""Repeat-rule arithmetic for task series, mirrored from Unify.

This is a deliberate copy of ``unify/task_scheduler/types/repetition.py``
(the recurrence subset: pattern schema, normalization, next-occurrence
projection, and deterministic dispatch jitter). Orchestra needs it so the
release path can re-project a future head for a repeating series whose worker
died before dispatch — the moment recurrence would normally advance.

Any change to the recurrence semantics MUST be applied to both files in the
same changeset, and both copies are pinned against each other by the shared
vector file ``repeat_projection_vectors.json`` (checked into each repo's test
tree with byte-identical content). Do not eliminate the duplication with a
cross-service call: projection runs inside Orchestra's write path and inside
Unify's dispatcher, and neither may block on the other.
"""
...
def parse_repeat_patterns(value: Any) -> list[RepeatPattern] | None:
    """Validate a stored ``repeat`` JSON list into repeat patterns.

    Task definition rows carry the patterns as the serialized dict list Unify
    wrote; ``None`` or an empty list means the task does not repeat.
    """

    if not value:
        return None
    return [RepeatPattern.model_validate(pattern) for pattern in value]
