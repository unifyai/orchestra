...
def seed_pool_numbers(dbsession: Session):
    ...
    if existing == 0:
        dbsession.add_all(
            [
                SharedPoolNumber(number="+15550100001", platform="whatsapp"),
                SharedPoolNumber(number="+15550100002", platform="whatsapp"),
            ],
        )
        dbsession.flush()
    yield
    ...
    async def test_conflict_events_list_endpoint(
        ...
        dbsession.add(
            ConflictEvent(
                ...
                old_pool_assignments={"1": "+15550100001"},
                new_pool_assignments={"1": "+15550100002"},
                status="resolved",
            ),
        )
        dbsession.commit()
...
class TestPoolNumberModelConstraints:
    def test_pool_numbers_seeded(self, pool_numbers):
        ...
        assert "+15550100001" in numbers
        assert "+15550100002" in numbers

    def test_pool_number_unique(self, dbsession: Session):
        dbsession.add(SharedPoolNumber(number="+15550100001", platform="whatsapp"))
...
class TestAdminEndpointFlows:
    ...
    async def test_resolve_unknown_sender_404(self, client: AsyncClient):
        response = await client.get(
            "/v0/admin/whatsapp/resolve",
            params={"pool_number": "+15550100001", "sender": "+15550000000"},
            headers=ADMIN_HEADERS,
        )
