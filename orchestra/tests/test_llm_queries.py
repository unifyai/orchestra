...
@pytest.mark.anyio
async def test_user_messages_not_string_concatenated_after_system_removal(
    client: AsyncClient,
):
    """
    When an inline system message appears between two user messages and is
    stripped (e.g., for Anthropic), the user messages should be combined as a
    list of content blocks, NOT string-concatenated.

    Bug: "greetings" + "pineapple" becomes "greetingspineapple" (no separator)
    Expected: The model should see two distinct user messages, not one merged word.

    The bug is most visible in reasoning/thinking blocks where the model shows
    what it received (e.g., "The user has sent 'greetingspineapple'").
    """
    endpoint = "/v0/chat/completions"
    data = {
        "model": "claude-3-haiku@anthropic",
        "messages": [
            {"role": "user", "content": "XRAY"},
            {
                "role": "system",
                "content": "This is an inline system message that will be stripped.",
            },
            {"role": "user", "content": "ZULU"},
        ],
        "temperature": 0.0,
        "max_tokens": 200,
        "stream": False,
    ...
    # Check reasoning_content if present (visible in extended thinking models)
    reasoning = message.get("reasoning_content", "") or ""

    # Check thinking_blocks if present
    thinking_blocks = message.get("thinking_blocks", []) or []
    thinking_text = " ".join(
        block.get("thinking", "") for block in thinking_blocks if block.get("thinking")
    )

    # Combine all text the model produced for checking
    all_model_text = f"{content} {reasoning} {thinking_text}".upper()

    # If the bug exists, the model sees "XRAYZULU" as one concatenated word.
    # If working correctly, it sees two separate items: "XRAY" and "ZULU".
    assert "XRAYZULU" not in all_model_text, (
        f"User messages were string-concatenated instead of being properly merged "
        f"as a list of content blocks. Full model output: {all_model_text}"
    )
