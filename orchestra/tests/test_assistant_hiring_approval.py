...
async def test_user_request_hiring_approval(client: AsyncClient):
    ...
    response = await client.post(
        "/v0/users/auth-user/assistant-hiring-approval", 
        headers=test_user["headers"]
    )
    assert response.status_code == status.HTTP_200_OK, response.json()
    data = response.json()
    assert data["message"] == "Request for assistant hiring submitted. You've been added to the waitlist."
    ...
    user_details = await client.get(f"/v0/admin/auth-user/by-user-id?user_id={test_user['id']}", headers=ADMIN_HEADERS)
...
async def test_admin_manage_hiring_approval(client: AsyncClient):
    ...
    response_user_req_approved = await client.post(user_request_url, headers=user_headers)
    ...
    # Verify status via admin GET
    user_details_resp_rejected = await client.get(f"/v0/admin/auth-user/by-user-id?user_id={user_id}", headers=ADMIN_HEADERS)
    ...
    response_user_req_after_reject = await client.post(user_request_url, headers=user_headers)
    assert response_user_req_after_reject.status_code == status.HTTP_200_OK
    assert "Request for assistant hiring submitted" in response_user_req_after_reject.json()["message"]
    assert response_user_req_after_reject.json()["assistant_hiring_approval"] == "pending"
    
    # Verify status is now pending via admin GET
    user_details_resp_pending_after_reject = await client.get(f"/v0/admin/auth-user/by-user-id?user_id={user_id}", headers=ADMIN_HEADERS)
    assert user_details_resp_pending_after_reject.status_code == status.HTTP_200_OK
    assert user_details_resp_pending_after_reject.json()["assistant_hiring_approval"] == "pending"
    ...
    # Verify status via admin GET
    user_details_resp_revoked = await client.get(f"/v0/admin/auth-user/by-user-id?user_id={user_id}", headers=ADMIN_HEADERS)
    ...
    response_user_req_after_revoke = await client.post(user_request_url, headers=user_headers)
    assert response_user_req_after_revoke.status_code == status.HTTP_200_OK
    assert "Request for assistant hiring submitted" in response_user_req_after_revoke.json()["message"]
    assert response_user_req_after_revoke.json()["assistant_hiring_approval"] == "pending"
...
async def test_admin_list_users_by_hiring_status(client: AsyncClient):
    ...
    # Set statuses
    await client.put(f"/v0/admin/auth-user/{user_pending['id']}/assistant-hiring-approval/pending", headers=ADMIN_HEADERS)
    await client.put(f"/v0/admin/auth-user/{user_approved['id']}/assistant-hiring-approval/approved", headers=ADMIN_HEADERS)
    await client.put(f"/v0/admin/auth-user/{user_rejected['id']}/assistant-hiring-approval/rejected", headers=ADMIN_HEADERS)
    await client.put(f"/v0/admin/auth-user/{user_revoked['id']}/assistant-hiring-approval/revoked", headers=ADMIN_HEADERS)
    # user_none will have assistant_hiring_approval = None by default

    list_url_base = "/v0/admin/auth-user/assistant-hiring-approval"

    # List pending
    response_pending_list = await client.get(f"{list_url_base}?status_filter=pending", headers=ADMIN_HEADERS)
    assert response_pending_list.status_code == status.HTTP_200_OK
    pending_users = response_pending_list.json()
    assert any(u["id"] == user_pending["id"] and u["assistant_hiring_approval"] == "pending" for u in pending_users)
    ...
    # List approved
    response_approved_list = await client.get(f"{list_url_base}?status_filter=approved", headers=ADMIN_HEADERS)
    assert response_approved_list.status_code == status.HTTP_200_OK
    approved_users = response_approved_list.json()
    assert any(u["id"] == user_approved["id"] and u["assistant_hiring_approval"] == "approved" for u in approved_users)
    assert not any(u["id"] == user_pending["id"] for u in approved_users)

    # List rejected
    response_rejected_list = await client.get(f"{list_url_base}?status_filter=rejected", headers=ADMIN_HEADERS)
    assert response_rejected_list.status_code == status.HTTP_200_OK
    rejected_users = response_rejected_list.json()
    assert any(u["id"] == user_rejected["id"] and u["assistant_hiring_approval"] == "rejected" for u in rejected_users)
    assert not any(u["id"] == user_pending["id"] for u in rejected_users)

    # List revoked
    response_revoked_list = await client.get(f"{list_url_base}?status_filter=revoked", headers=ADMIN_HEADERS)
    assert response_revoked_list.status_code == status.HTTP_200_OK
    revoked_users = response_revoked_list.json()
    assert any(u["id"] == user_revoked["id"] and u["assistant_hiring_approval"] == "revoked" for u in revoked_users)
    assert not any(u["id"] == user_pending["id"] for u in revoked_users)
    
    # List users with no status (None)
    response_none_list = await client.get(f"{list_url_base}?status_filter=none", headers=ADMIN_HEADERS)
    assert response_none_list.status_code == status.HTTP_200_OK
    none_status_users = response_none_list.json()
    assert any(u["id"] == user_none["id"] and u["assistant_hiring_approval"] is None for u in none_status_users)
    assert not any(u["id"] == user_pending["id"] for u in none_status_users)

    # List all
    response_all_list = await client.get(f"{list_url_base}?status_filter=all", headers=ADMIN_HEADERS)
...
async def test_one_time_approval_links_flow(client: AsyncClient):
    ...
    assert response_create_link.status_code == status.HTTP_201_CREATED, response_create_link.json()
    ...
    user_details = await client.get(f"/v0/admin/auth-user/by-user-id?user_id={user_id}", headers=ADMIN_HEADERS)
    ...
    assert credits_after_first_claim == expected_credits_after_first_claim, \
    ...
    credits_after_second_claim_attempt = await get_credits(client, user_headers=user_headers)
    assert credits_after_second_claim_attempt == credits_after_first_claim, \
        "Credits should not change on subsequent claims of the same link by the same user."

    # Admin lists links, check if claimed
    response_list_links = await client.get("/v0/admin/assistant-hiring-one-time-link", headers=ADMIN_HEADERS)
    assert response_list_links.status_code == status.HTTP_200_OK
    links = response_list_links.json()
    claimed_link_in_list = next((l for l in links if l["token"] == one_time_token), None)
    ...
    response_delete_link = await client.delete(f"/v0/admin/assistant-hiring-one-time-link/{link_id_db}", headers=ADMIN_HEADERS)
...
async def test_one_time_link_single_benefit_and_multiple_links(client: AsyncClient):
    ...
    user1_details_before = await client.get(f"/v0/admin/auth-user/by-user-id?user_id={user1_id}", headers=ADMIN_HEADERS)
    assert user1_details_before.json()["assistant_hiring_approval"] is None # Or "pending" if they requested
    assert user1_details_before.json()["has_claimed_approval_link"] is False # Assuming AuthUserResponse includes this

    # Admin creates Link L1
    resp_l1 = await client.post("/v0/admin/assistant-hiring-one-time-link", json={"expires_in_days": 1}, headers=ADMIN_HEADERS)
    assert resp_l1.status_code == status.HTTP_201_CREATED
    l1_token = resp_l1.json()["token"]

    # User 1 claims Link L1
    resp_u1_claim_l1 = await client.post("/v0/users/auth-user/claim-assistant-hiring-one-time-link", json={"token": l1_token}, headers=user1_headers)
    ...
    user1_details_after_l1 = await client.get(f"/v0/admin/auth-user/by-user-id?user_id={user1_id}", headers=ADMIN_HEADERS)
    ...
    resp_u1_reclaim_l1 = await client.post("/v0/users/auth-user/claim-assistant-hiring-one-time-link", json={"token": l1_token}, headers=user1_headers)
    ...
    resp_l2 = await client.post("/v0/admin/assistant-hiring-one-time-link", json={"expires_in_days": 1}, headers=ADMIN_HEADERS)
    assert resp_l2.status_code == status.HTTP_201_CREATED
    l2_token = resp_l2.json()["token"]
    l2_id = resp_l2.json()["id"]

    resp_u1_claim_l2 = await client.post("/v0/users/auth-user/claim-assistant-hiring-one-time-link", json={"token": l2_token}, headers=user1_headers)
    ...
    link_l2_details = await client.get(f"/v0/admin/assistant-hiring-one-time-link", headers=ADMIN_HEADERS) # List all links
    l2_from_list = next((link for link in link_l2_details.json() if link["id"] == l2_id), None)
    assert l2_from_list is not None
    assert l2_from_list["user_id"] is None

    # --- Admin revokes User 1's approval ---
    await client.put(f"/v0/admin/auth-user/{user1_id}/assistant-hiring-approval/revoked", headers=ADMIN_HEADERS)
    user1_details_revoked = await client.get(f"/v0/admin/auth-user/by-user-id?user_id={user1_id}", headers=ADMIN_HEADERS)
    ...
    resp_u1_claim_l2_after_revoke = await client.post("/v0/users/auth-user/claim-assistant-hiring-one-time-link", json={"token": l2_token}, headers=user1_headers)
    ...
    assert resp_u1_claim_l2_after_revoke.json()["assistant_hiring_approval"] == "approved"

    # Verify User 1 credits did NOT change, Link L2 STILL unclaimed
    credits_u1_after_l2_revoke_attempt = await get_credits(client, user_headers=user1_headers)
    assert credits_u1_after_l2_revoke_attempt == credits_u1_after_l1
    user1_details_after_l2_revoke_claim = await client.get(f"/v0/admin/auth-user/by-user-id?user_id={user1_id}", headers=ADMIN_HEADERS)
    assert user1_details_after_l2_revoke_claim.json()["assistant_hiring_approval"] == "approved"
    
    link_l2_details_after = await client.get(f"/v0/admin/assistant-hiring-one-time-link", headers=ADMIN_HEADERS)
    l2_from_list_after = next((link for link in link_l2_details_after.json() if link["id"] == l2_id), None)
...
async def test_claim_invalid_one_time_link(client: AsyncClient):
    ...
    create_link_resp = await client.post("/v0/admin/assistant-hiring-one-time-link", json={"expires_in_days": 1}, headers=ADMIN_HEADERS)
    ...
    # User A claims it
    await client.post("/v0/users/auth-user/claim-assistant-hiring-one-time-link", json={"token": link_token_for_A}, headers=user_A["headers"])
    ...
    assert "already been claimed by another user" in response_user_B_claim.json()["detail"]

    # Verify User B's credits did not change
    user_B_credits_after_failed_attempt = await get_credits(client, user_headers=user_B["headers"])
