...
async def test_train_router(client: AsyncClient):
    ...
    dataset = _upload_dataset(client, dataset_name=dataset_name, data_path=sample_path)
    params = {"name": router_name, "dataset": dataset_name, "endpoints": endpoints}
    response = await client.post("/v0/dataset", data=data, files=files, headers=HEADERS)
    assert response.status_code == 200

    # how do you check if it actually trains?
    for tries in range(30*60):
        url = "/v0/router/train/list"
        response = await client.get(url, headers=HEADERS)
        if router_name not in response:
            asyncio.sleep(60)
