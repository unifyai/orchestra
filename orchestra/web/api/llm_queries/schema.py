...
class ChatCompletionRequest(BaseModel):
    ...
    model: str = Field(description="The endpoint to use, in the format "
                       "<model>@<provider>, based on any of the supported endpoints as "
                       "per the list returned by `/v0/endpoints`",
    ...
    temperature: float = Field(
        0.9, description="What sampling temperature to use, between 0 and 2. Higher "
        "values like 0.8 will make the output more random, while lower values like 0.2 "
        "will make it more focused and deterministic. Generally recommended to alter "
        "this or `top_p`, but not both.", json_schema_extra={"example": 0.9})
    stream: bool = Field(
    ...
    max_tokens: Optional[int] = Field(
        ...
        "chat completion. The total length of input tokens and generated tokens is "
    ...
    logit_bias: Optional[Dict[str, float]] = Field(
        None, description="Modify the likelihood of specified tokens appearing in the "
    ...
    logprobs: Optional[bool] = Field(
        None, description="Whether to return log probabilities of the output tokens or "
        "not. If true, returns the log probabilities of each output token returned in "
        "the `content` of `message`.", json_schema_extra={"example": False})
    top_logprobs: Optional[int] = Field(
        None, description="An integer between 0 and 20 specifying the number of most "
        "likely tokens to return at each token position, each with an associated log "
        "probability. logprobs must be set to true if this parameter is used.",
        json_schema_extra={"example": 15})
    n: Optional[int] = Field(
        None, description="How many chat completion choices to generate for each input "
    ...
    response_format: Optional[Dict[str, str]] = Field(
        ...
        'schema. Learn more in the Structured Outputs guide. Setting to { "type": '
        '"json_object" } enables JSON mode, which ensures the message the model '
        'generates is valid JSON. Important: when using JSON mode, you must also '
        ...
        'the generation reaches the token limit, resulting in a long-running and '
    ...
    top_p: Optional[float] = Field(
        ...
        "probability mass are considered. Generally recommended to alter this or "
        "`temperature` but not both.", json_schema_extra={"example": 0.5})
    tools: Optional[List[Any]] = Field(
        None, description="A list of tools the model may call. Currently, only "
    ...
    tool_choice: Optional[Any] = Field(
        None, description="Controls which (if any) tool is called by the model. `none` "
        ...
        "tools are present.", json_schema_extra={
