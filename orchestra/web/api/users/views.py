...
from orchestra.db.dao.auth_user_dao import AuthUserDAO, ASSISTANT_HIRING_APPROVAL_STATUSES
from orchestra.db.dao.organization_dao import OrganizationDAO
from orchestra.db.dao.organization_member_dao import OrganizationMemberDAO
from orchestra.db.dao.users_dao import UsersDAO
from orchestra.db.dao.assistant_hiring_one_time_approval_link_dao import AssistantHiringOneTimeApprovalLinkDAO
...
@router.post("/auth-user/assistant-hiring-approval", response_model=AssistantHiringApprovalResponse, status_code=200)
async def request_assistant_hiring_approval(
    ...
    if current_status == "approved" or current_status == "pending":
        return AssistantHiringApprovalResponse(message=f"Assistant hiring {current_status}.", assistant_hiring_approval=current_status)

    if auth_user_dao.set_assistant_hiring_approval(user_id, "pending"):
        session.commit()
        return AssistantHiringApprovalResponse(message="Request for assistant hiring submitted. You've been added to the waitlist.", assistant_hiring_approval="pending")
    else:
        session.rollback()
        raise HTTPException(status_code=500, detail="Failed to update your hiring approval request status.")

@admin_router.put("/auth-user/{target_user_id}/assistant-hiring-approval/{status}", response_model=AssistantHiringApprovalResponse)
async def set_user_assistant_hiring_status(
    ...
    if status not in ASSISTANT_HIRING_APPROVAL_STATUSES or status is None:
        raise HTTPException(status_code=500, detail=f"Invalid status. Must be one of {', '.join(s for s in ASSISTANT_HIRING_APPROVAL_STATUSES if s is not None)}.")
    ...
    if auth_user_dao.set_assistant_hiring_approval(target_user_id, status):
        session.commit()
        return AssistantHiringApprovalResponse(message=f"User {target_user_id} assistant hiring approval status set to '{status}'.", assistant_hiring_approval=status)
    else:
        session.rollback()
        raise HTTPException(status_code=500, detail=f"Failed to set hiring approval status for user {target_user_id}.")
    
@admin_router.get("/auth-user/assistant-hiring-approval", response_model=List[AssistantHiringApprovalUserStatus])
async def list_users_by_assistant_hiring_approval(
    status_filter: Optional[str] = Query(None, description=f"Filter by status: {', '.join(s for s in ASSISTANT_HIRING_APPROVAL_STATUSES if s is not None)} or 'all'"),
    session: Session = Depends(get_db_session),
):
    auth_user_dao = AuthUserDAO(session)
    ...
    if not status_filter or status_filter.lower() == "all":
        ...
        users_to_list = auth_user_dao.get_users_by_assistant_hiring_approval(status_filter)
...
@router.post("/auth-user/claim-assistant-hiring-one-time-link", response_model=AssistantHiringApprovalResponse, status_code=200)
async def claim_assistant_hiring_one_time_link(
    ...
    if user.has_claimed_approval_link:
        ...
        if user.assistant_hiring_approval == "revoked" or user.assistant_hiring_approval == "rejected":
        ...
        return AssistantHiringApprovalResponse(message=message, assistant_hiring_approval="approved")
    
    # Check if link already used
    if link.user_id is not None:
        if link.user_id == user_id:
            return AssistantHiringApprovalResponse(message="You already used this approval link.", assistant_hiring_approval=user.assistant_hiring_approval)
        else:
            raise HTTPException(status_code=400, detail="Approval link has already been claimed.")
    ...
    try:
        ...
        if not claimed_link:
            session.rollback()
            raise HTTPException(status_code=400, detail="Failed to claim approval link. It might be invalid, expired or already claimed.")
        ...
        # Grant credits
        users_dao.recharge_credit(user_id=user.id, quantity=float(ASSISTANT_CREATION_COST))
        ...
        session.rollback()
        raise HTTPException(status_code=500, detail=f"An unexpected error occurred during link processing: {str(e)}")

@admin_router.post("/assistant-hiring-one-time-link", response_model=AssistantHiringOneTimeLinkResponse, status_code=201)
...
@admin_router.get("/assistant-hiring-one-time-link", response_model=List[AssistantHiringOneTimeLinkResponse])
