...
def update_assistant_config(
    ...
    # Use exclude_unset=True to get a dictionary of only the fields provided in the request
    update_data = update.dict(exclude_unset=True)
    create_infra = update_data.pop("create_infra", update.create_infra)

    # Determine if any contact info fields are part of the update
    contact_info_updated = any(
        k in update_data
        for k in ["phone", "email", "user_whatsapp_number", "user_phone"]
    )

    # Store old GCS URLs for potential cleanup
    old_photo_url = existing_assistant.profile_photo
    old_video_url = existing_assistant.profile_video

    # Variables for infrastructure rollback
    ...
    try:
        ...
        if create_infra:
            ...
            if "email" in update_data and not existing_assistant.email:
                try:
                    ...
                    raise HTTPException(
                        status_code=500,
                        detail=f"Failed to create and watch email: {str(e)}",
                    )
            ...
            if "user_phone" in update_data and not existing_assistant.phone:
                try:
                    ...
                    raise HTTPException(
                        status_code=500,
                        detail=f"Failed to create phone number: {str(e)}",
                    )
        ...
        # Handle type conversion for Decimal fields before passing to DAO
        ...
        # Call the DAO with the constructed dictionary of changes
