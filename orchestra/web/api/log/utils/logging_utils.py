...
def _get_logs_query_jsonb(
    ...
    Key differences from _get_logs_query:
    - No EAV table joins (Log, LogEventLog, DerivedLog)
    ...
    if sorting:
        ...
        if is_vector_sort:
            # --- Vector ANN fast-path ---
            # This mirrors the EAV path's vector sorting implementation
            try:
    ...
    # =========================================================================
    if latest_timestamp:
        # Use updated_at to match EAV behavior - this changes after log updates
...
def _create_logs_internal_jsonb(
    ...
    # Reuse the same logic as EAV path for extracting auto-counting/unique key values
...
def _format_jsonb_logs(
    ...
    Note:
        JSONB mode doesn't support context versions, so response omits the 'versions' key
        that EAV includes for context-based versioning.
...
def _build_log_subquery_jsonb(
    ...
    """
    JSONB version: Build subquery selecting log_event_id and data column.

    This eliminates the lateral subquery explosion of the EAV version by selecting
    the entire JSONB data column in a single query instead of per-field joins.
...
def _create_logs_from_joined_rows_jsonb(
    ...
    """
    JSONB version: Insert merged JSONB directly into LogEvent.data.

    This simplified version eliminates all EAV table dependencies by storing
    the merged JSONB directly in LogEvent.data. No Log/DerivedLog creation needed.
...
def _join_logs_jsonb(
    ...
    efficient field merging, eliminating the EAV lateral subquery explosion.
