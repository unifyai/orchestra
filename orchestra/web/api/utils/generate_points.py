    ...
    for dataset in datasets:
        ...
        for benchmark in datasets[dataset]:
            if benchmark["prompt"] in prompts:
                prompts[benchmark["prompt"]][f"{benchmark['mdl_name']}_score"] = benchmark["score"]
                prompts[benchmark["prompt"]][f"{benchmark['mdl_name']}_pred"] = benchmark["pred"]
            else:
...
def generate_router_points(data):
    ...
    for dataset in data:
        ...
        endpoint_scores = {
            endpoint: get_mean(endpoint)
            for endpoint in endpoints
        }
        endpoint_scores = {
            endpoint: score for endpoint, score in endpoint_scores.items()
            if score != None
        }
        ...
        cost_multiplier = mean([endpoint_scores[endpoint] / endpoint_costs[endpoint]  for endpoint in current_endpoints])
        ttft_multiplier = mean([endpoint_scores[endpoint] / endpoint_ttft[endpoint] for endpoint in current_endpoints])
        itl_multiplier = mean([endpoint_scores[endpoint] / endpoint_itl[endpoint] for endpoint in current_endpoints])
        
        num = 20
        a_values, b_values, c_values = [], [], []
        a_start, b_start, c_start = math.log10(cost_multiplier * 3), math.log10(ttft_multiplier * 3), math.log10(itl_multiplier * 3)
        end = math.log10(1e-8)
        a_step, b_step, c_step = (end - a_start) / (num - 1), (end - b_start) / (num - 1), (end - b_start) / (num - 1)
        values = [(
            10 ** (a_start + a_step * i), 
            10 ** (b_start + b_step * i), 
            10 ** (c_start + c_step * i)
        ...
        for a_idx, a in enumerate(a_values):
            for b_idx, b in enumerate(b_values):
                for c_idx, c in enumerate(c_values):
                        ...
                        prompt_max.append(
                            {
                                ...
                                "score": prompt_list[i][
                                    endpoint_to_model(max_endpoint).replace("_pred", "") + "_score"
                                ]
                            }
                        )
                    ...
                    router_scores = {endpoint: router_scores[endpoint] / router_counts[endpoint] for endpoint in router_scores}
                    router_cost = {endpoint: router_cost[endpoint] / router_counts[endpoint] for endpoint in router_cost}
                    router_ttft = {endpoint: router_ttft[endpoint] / router_counts[endpoint] for endpoint in router_ttft}
                    router_itl = {endpoint: router_itl[endpoint] / router_counts[endpoint] for endpoint in router_itl}
                    total_weight = sum(router_counts.values())
                    final_score = sum([router_scores[endpoint] * router_counts[endpoint] for endpoint in router_scores]) / total_weight
                    final_cost = sum([router_cost[endpoint] * router_counts[endpoint] for endpoint in router_cost]) / total_weight
                    final_ttft = sum([router_ttft[endpoint] * router_counts[endpoint] for endpoint in router_ttft]) / total_weight
                    final_itl = sum([router_itl[endpoint] * router_counts[endpoint] for endpoint in router_itl]) / total_weight
...
def get_point_solutions(data):
    ...
    for dataset in data:
        prompt_list = data[dataset]
        models = [key.replace("_score", "") for key in prompt_list[0].keys() if key != "prompt" and "pred" not in key]
        model_scores = dict()
        model_counts = dict()
        ...
        dataset_scores = []
        for model in models:
            dataset_scores.append({"model": model, "quality": round(model_scores[model] / model_counts[model], 2)})
...
def prune_router_points(router_points):
    ...
    for dataset in router_points:
        print(dataset)
        points = [[point["quality"], point["cost"], point["ttft"], point["itl"]] for point in router_points[dataset]]
        if points:
            try:
            ...
            metrics = np.array([[point["quality"], point["cost"], point["ttft"], point["itl"]] for point in convex_points])
            normalized_metrics = (metrics - np.mean(metrics, axis=0)) / np.std(metrics, axis=0)
            ...
            for i in range(num):
                if i not in deleted:
                    for j in range(num):
                        if dist_matrix[i][j] < threshold and i != j and j not in deleted:
                            deleted.append(j)
            
            print(len(deleted), num)
            pruned_points = [point for i, point in enumerate(convex_points) if i not in deleted]
