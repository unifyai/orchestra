...
``@tenant.onmicrosoft.com`` (Microsoft 365) mailboxes are being
...
- ``UNIFY_COMMS_URL`` — base URL of the Communication service
...
DOMAIN_TO_PROVIDER: dict[str, str] = {
    ...
    "tenant.onmicrosoft.com": "microsoft_365",
}
...
async def amain(args: argparse.Namespace) -> int:
    ...
    if args.apply:
        if not os.environ.get("UNIFY_COMMS_URL"):
            logger.error("UNIFY_COMMS_URL is not set; aborting.")
            return 2
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    p = argparse.ArgumentParser(
        ...
        description=(
            "Decommission platform-issued assistant mailboxes "
            "(@unify.ai / @tenant.onmicrosoft.com). "
