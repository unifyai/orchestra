...
async def main():
    ...
    try:
        ...
        if not DEBUG:
            ...
            # naive check if we got any judgements
            # TODO: make this more sophisticated
            status = "completed" if id_to_model_to_scores else "failed"
            log_msg(f"Marking task as {status}")
            mark_status(api_key, name, user_id, status)
        ...
        log_msg("Failed Benchmark")
        log_msg(e)
        mark_status(api_key, name, user_id, status="failed")
