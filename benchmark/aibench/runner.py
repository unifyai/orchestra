...
class AIBenchRunner:
    ...
    def __init__(self, fn, load, input_policy, seed=42):
        """
        Initialize the AIBenchRunner.

        :param fn: The function to benchmark. Takes as input a string and returns a string asynchronously (streaming).
        :param load: The number of concurrent requests to run.
        :param input_policy: The input policy to use (short or long).
    ...
    def itl(self):
        """
        Compute the input tokens per second.

        :return: The input tokens per second.
        """
        return [
    ...
    def output_tks_per_sec(self):
        """
        Compute the output tokens per second.

        :return: The output tokens per second.
        """
        return [1000 / i for i in self.itl]

    def as_dict(self):
        """
        Return the metrics such as itl, cold_start etc as a dictionary.
    ...
    async def compute_metrics(self):
        """Compute the metrics for a single request."""
    ...
    async def check_coldstart(self, threshold):
        ...
        :param threshold: The threshold for the cold start.

        :return: The cold start time.
        """
        prompt = "2+2 is "
        completions = []
