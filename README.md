...
1. To setup the database locally, you need to create a dump file of the staging database. To do so, you can either run the following commands:
    ...
    - After connecting to staging, run the following command on your terminal. This will create a dump file.
        ```bash
        pg_dump -h 203.0.113.10 -p 5432 -U postgres orchestra > orchestra.sql
        ```
    OR
    - Use GCP UI to pg_dump the data through the [export page here](https://console.cloud.google.com/sql/instances/dev/export?project=gcp-project-saas) into [temp_file_holder](https://console.cloud.google.com/storage/browser/temp_file_holder;tab=objects?forceOnBucketsSortingFiltering=true&project=gcp-project-saas&prefix=&forceOnObjectsSortingFiltering=false) bucket and then download from there.
