...
class DCN(nn.Module):
    def __init__(
        ...
        self.prompt_proj = nn.Linear(d_cn, d_cn - embed_dim)
        self.dev = device
    ...
    def get_prompt_embs(self, datum_id, attn_mask):
        datum_id = datum_id.to(self.dev)
        prompt_emb = self.prompt_encoder(
            input_ids=datum_id,
            attention_mask=attn_mask.to(self.dev),
    ...
    def get_latent_rep(self, model_id):
        model_id = model_id.to(self.dev)
        model_emb = self.model_embedding(model_id)
        return model_emb

    def embs_to_preds(self, x, model_emb):
        x_cat = torch.cat([x, model_emb], dim=-1)
        x = self.cn(x_cat)
        return x
